...
from PyQt5.QtWidgets import (
    ...
    QLabel, QLineEdit, QMessageBox, QTreeWidget, QTreeWidgetItem, QSplitter, QDialog, QGraphicsScene, QGraphicsView
)
from PyQt5.QtCore import Qt
from PyQt5.QtGui import QPixmap, QImage

# ------------------- 配置 -------------------
EXPIRE_DAYS = 1  # 程序过期天数
ENCRYPTED_FOLDER = "encrypted_files"  # 启动时读取该目录
SECRET_KEY = "MySecretKey123"  # 用于生成验证码
LOGO_PATH = "../logo.png"  # 临时 logo 图片路径
# -----------------------------------------
...
class PDFViewer(QWidget):
    def __init__(self):
        ...
        splitter.setStretchFactor(1, 5)  # PDF 内容尽可能大
    ...
    def open_pdf(self, item):
        ...
        try:
            ...
            QMessageBox.warning(self, "错误", f"无法解密 PDF: {str(e)}")
            return

        tmp_pdf = os.path.join(tempfile.gettempdir(), f"tmp_{uuid.uuid4().hex}.pdf")
        ...
        if doc.page_count == 0:
            QMessageBox.warning(self, "错误", "PDF 文件为空")
            return
        ...
        self.pdf_view_widget.fitInView(self.pdf_scene.itemsBoundingRect(), Qt.KeepAspectRatio)
        self._zoom_factor = 1.0  # 默认显示比例 100%

    def zoom_in(self):
        self._zoom_factor *= 1.2
        self.pdf_view_widget.resetTransform()
        self.pdf_view_widget.scale(self._zoom_factor, self._zoom_factor)
...
def main():
    ...
    auth = AuthWindow()
    if auth.exec_() != QDialog.Accepted or not auth.accepted_flag:
        sys.exit()
